...
start "PDF-App Frontend" cmd /k "cd frontend && npx expo start"
goto end

:both
echo.
echo Starting Backend Server...
start "GDP Backend" cmd /k "cd gdp-backend && npm start"
timeout /t 3 >nul
echo Starting Frontend App...
start "GDP Frontend" cmd /k "cd gdp-app && npx expo start"
goto end

:both_debug
echo.
echo Starting Backend Server in DEBUG mode...
start "GDP Backend DEBUG" cmd /k "cd gdp-backend && npm run debug"
timeout /t 3 >nul
echo Starting Frontend App...
start "GDP Frontend" cmd /k "cd gdp-app && npx expo start"
goto end
